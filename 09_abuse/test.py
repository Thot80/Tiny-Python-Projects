...
def test_bad_adjective_str():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f'{win_prg} -a {bad}')
    assert rv != 0
...
def test_bad_adjective_num():
    ...
    rv, out = getstatusoutput(f'{win_prg} -a {n}')
    print(out)
    assert rv != 0
...
def test_bad_number_str():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f'{win_prg} -n {bad}')
    assert rv != 0
    assert re.search(f"invalid int value: '{bad}'", out)
...
def test_bad_number_int():
    ...
    n = random.choice(range(-10, 0))
    rv, out = getstatusoutput(f'{win_prg} -n {n}')
    assert rv != 0
...
def test_bad_seed():
    ...
    bad = random_string()
    rv, out = getstatusoutput(f'{win_prg} -s {bad}')
    assert rv != 0
    assert re.search(f"invalid int value: '{bad}'", out)
...
def test_01():
    ...
    out = getoutput(f'{win_prg} -s 1 -n 1')
...
def test_02():
    ...
    out = getoutput(f'{win_prg} --seed 2')
    expected = """
...
def test_03():
    ...
    out = getoutput(f'{win_prg} -s 3 -n 5 -a 1')
    expected = """
...
def test_04():
    ...
    out = getoutput(f'{win_prg} --seed 4 --number 2 --adjectives 4')
    expected = """
